...
/// <summary>集中构造 Windows Shell 启动参数，便于在不真正拉起游戏的情况下验证安全边界。</summary>
internal static class WindowsGameLaunchStartInfoFactory
{
    internal static ProcessStartInfo Create(GameLaunchProfile profile) => profile.TargetType switch
    {
        GameLaunchTargetType.Executable => CreateExecutable(profile),
        GameLaunchTargetType.Shortcut => CreateShortcut(profile),
        GameLaunchTargetType.StoreUri => CreateStoreUri(profile),
        _ => throw new ArgumentOutOfRangeException(nameof(profile.TargetType), profile.TargetType, "不支持的游戏启动入口类型。")
    };

    private static ProcessStartInfo CreateExecutable(GameLaunchProfile profile)
    {
    ...
    private static ProcessStartInfo CreateShortcut(GameLaunchProfile profile)
    {
        ...
        return new ProcessStartInfo
        {
            FileName = target,
            Arguments = profile.Arguments ?? string.Empty,
            WorkingDirectory = GetWorkingDirectory(profile.WorkingDirectory, target),
            UseShellExecute = true,
            Verb = profile.RunAsAdministrator ? "runas" : "open"
        };
    }

    private static ProcessStartInfo CreateStoreUri(GameLaunchProfile profile)
    {
        ValidateArguments(profile.Arguments);
        ...
        return new ProcessStartInfo(profile.Target)
        {
            Arguments = profile.Arguments ?? string.Empty,
            UseShellExecute = true,
            Verb = "open"
