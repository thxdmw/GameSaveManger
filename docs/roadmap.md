...
项目 C# XML 注释和 项目文档统一使用中文；代码标识符、协议字段和稳定错误码继续使用英文。
...
- 轻量 JSON Lines 文件日志、常见凭据脱敏、14 天本地保留和未处理异常记录。
- 仅针对幂等 GET 的短暂网络故障重试；上传、删除和快照提交等写操作不自动重放。
- SQLite `schema_version` 表、事务化迁移、旧 `sync_state` 的安全重建和未来版本数据库保护。
- Windows x64/ARM64 自包含单文件发布脚本、Inno Setup 安装器定义及中文发布手册。
- 本机已完成 `win-x64` 自包含与 framework-dependent 单文件发布验证：分别约 135.4 MiB 与 2.5 MiB。

仍需外部发布条件：

- 代码签名需要受保护的证书与时间戳服务，不能写入代码仓库。
- 自动更新需要 HTTPS 发布源、已签名的版本清单和回滚策略，待发布基础设施确定后接入。
- CMS Docker Compose 冒烟链路需要本机 Docker 服务可用后执行。